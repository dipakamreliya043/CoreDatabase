...
class WallStreetVC: UIViewController {

    //Outlet
    @IBOutlet weak var tblView: UITableView!
    
    var arrWallStreet = [Article]()
...
extension WallStreetVC: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return arrWallStreet.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: Story_Cell = tblView.dequeueReusableCell(withIdentifier: "Story_Cell", for: indexPath) as! Story_Cell
        if arrWallStreet.count > 0{
            if let dict = arrWallStreet[indexPath.row] as? Article{
...
//MARK: Api Call
extension WallStreetVC {
    func GetApiWallStreetVC() {
        let apiName = getDomainApi()
                ...
                if let arrDict = dictData["articles"] as? [[String : Any]] {
                    ...
                    for i in 0..<arrHomes.count {
                        if let dict = arrHomes[i] as? Article{
                            self.insertData(author: dict.author ?? "No Name", newtype: "WallStreet", title: dict.title ?? "No Title", isLike: false, urlToImage: dict.urlToImage ?? "", publishedAt: dict.publishedAt ?? "")
                        }
                    }
                    self.retriveingData()
                }
...
extension WallStreetVC {
    ...
    func retriveingData() {
        ...
        fetchRequest.predicate = NSPredicate(format: "newtype = %@", "WallStreet")
        
        do {
            let results = try context.fetch(fetchRequest)
            self.arrWallStreet = [Article]()
            for i in 0..<results.count {
                let dict = results[i]
                self.arrWallStreet.append(Article(fromDictionary: ["id": dict.id ?? UUID(), "author": dict.author ?? "", "title": dict.title ?? "", "urlToImage": dict.urlToImage ?? "", "publishedAt": dict.publishedAt ?? "", "isLike": dict.isLike]))
            }
            self.reloadData()
        } catch {
            print("Error")
    ...
    func deleteData(id: UUID, index: Int) {
        ...
        do {
            try context.save()
            self.arrWallStreet.remove(at: index)
            reloadData()
        } catch {
            //Handle error
        }
    ...
    func likePost(id: UUID, index: Int, isLike: Bool) {
        ...
        if let result = try? context.fetch(fetchRequest) {
            ...
            do {
                try context.save()
                self.arrWallStreet[index].isLike = isLike
